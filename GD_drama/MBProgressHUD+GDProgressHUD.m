...
#import "GDActivityIndicatorView.h"
...
+ (MBProgressHUD *)showLoadingState:(NSString *)title inView:(UIView *)view inBackground:(BOOL)dimBackground
{
    ...
    if (view) {
        MBProgressHUD *progress = [[MBProgressHUD alloc] initWithView:view];
        progress.mode = MBProgressHUDModeCustomView;
        progress.customView = [[GDActivityIndicatorView alloc] initWithFrame:CGRectMake(view.center.x - indicatorRadius, view.center.y - indicatorRadius, indicatorRadius * 2, indicatorRadius * 2)];
        progress.removeFromSuperViewOnHide = YES;
